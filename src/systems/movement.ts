...
export default function createMovementSystem(world: World, app: Application) {
    ...
    return () => {
        for (const entity of movableEntities.get()) {
            if (
                entity.position.x <= 0 ||
                entity.position.x >= app.screen.width
            ) {
            ...
            if (
                entity.position.y <= 0 ||
                entity.position.y >= app.screen.height
            ) {
                ...
                entity.velocity.direction =
                    Math.PI * 2 - entity.velocity.direction;
            }

            entity.position.x +=
                Math.round(Math.cos(entity.velocity.direction) * entity.velocity.speed);
            entity.position.y +=
                Math.round(Math.sin(entity.velocity.direction) * entity.velocity.speed);

            const rotation = Math.PI * entity.velocity.rotationFactor;
            entity.velocity.direction +=
